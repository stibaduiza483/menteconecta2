...
fun PsiquiatrasScreen(navController: NavController) {
        ...
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (data in snapshot.children) {
                    ...
                    // Filtro estricto por la especialidad del archivo
                    if (esp.equals("Psiquiatra", ignoreCase = true)) {
    ...
    Column(modifier = Modifier.fillMaxSize().background(fondoApp).padding(16.dp)) {
        ...
        Text("Especialidad: Psiquiatría", fontSize = 14.sp, color = Color.Gray)
        ...
        if (cargando) {
            ...
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                Text("No hay citas de psiquiatría disponibles.", color = Color.Gray)
            }
        } else {
